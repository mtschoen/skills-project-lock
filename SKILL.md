...
- **unity-batchmode-worktree** acquires this lock on the agent's paired worktree before writing, alongside its own `.worktree-reserved` pool-slot marker (that marker tracks pool membership; this lock coordinates the writes). The marker is self-documenting rather than a bare touch file: its contents record `reserved-by` (the claiming session) and `reserved-at` (an ISO8601 timestamp), with a `stale-after` window (24h) after which an abandoned marker is safe to reap.
