...
Direct file-tool writes to Git administration state are denied regardless of which lock the session holds, because that state is shared by every worktree of the repository and no single worktree's lock speaks for it. The check covers the paths reported by `git rev-parse --git-dir` and `--git-common-dir` plus the `.git` marker at a worktree root, and short-circuits unless a path component ends in `.git` so ordinary edits pay no subprocess cost. Bash is excluded on purpose: Git writes to `.git` on nearly every invocation.
